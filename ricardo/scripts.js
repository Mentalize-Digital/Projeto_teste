...
async function loadChecklist() {
    try {
        ...
        const savedChecklist = localStorage.getItem('ricardo-checklist');
...
function showCelebration() {
    ...
    if (progressText && !progressText.classList.contains('celebrated')) {
        ...
        setTimeout(() => {
            ...
            const items = JSON.parse(localStorage.getItem('ricardo-checklist') || '[]');
            const completed = items.filter(item => item.completed).length;
            const total = items.length;
...
function updateChecklistItem(index, completed) {
    ...
    if (savedChecklist) {
        ...
        localStorage.setItem('ricardo-checklist', JSON.stringify(data));
...
async function resetChecklist() {
    if (confirm('Tem certeza que deseja resetar todo o progresso do checklist?')) {
        try {
            // Limpar localStorage
            localStorage.removeItem('ricardo-checklist');
