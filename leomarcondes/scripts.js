...
async function loadChecklist() {
    try {
        ...
        const savedChecklist = localStorage.getItem('leomarcondes-checklist');
...
function showCelebration() {
    ...
    if (progressText && !progressText.classList.contains('celebrated')) {
        ...
        setTimeout(() => {
            ...
            const items = JSON.parse(localStorage.getItem('leomarcondes-checklist') || '[]');
            const completed = items.filter(item => item.completed).length;
            const total = items.length;
...
function updateChecklistItem(index, completed) {
    ...
    if (savedChecklist) {
        ...
        localStorage.setItem('leomarcondes-checklist', JSON.stringify(data));
...
async function resetChecklist() {
    if (confirm('Tem certeza que deseja resetar todo o progresso do checklist?')) {
        try {
            // Limpar localStorage
            localStorage.removeItem('leomarcondes-checklist');
...
async function loadMonitoringData() {
    try {
        ...
        displayMonitoringKPIs(data.kpis_principais);
        displayPriorityActions(data.alertas_criticos);
...
function displayMonitoringKPIs(kpisData) {
    ...
    // Determinar status baseado no valor atual vs meta
    function getStatus(atual, meta) {
        if (atual === 'N/A') return 'pendente';
        // Lógica simples: se tem valor numérico, comparar
        if (atual.includes('R$') || atual.includes('%') || !isNaN(parseFloat(atual))) {
            return 'ok';
        }
        return 'pendente';
    }

    container.innerHTML = kpisData.map(categoria => `
        <div class="bg-white/5 border border-white/10 rounded-lg p-4">
            <h3 class="text-white font-semibold text-base mb-3">${categoria.categoria}</h3>
            <div class="flex flex-col gap-2">
                ${categoria.indicadores.map(indicador => {
                    const status = getStatus(indicador.atual, indicador.meta);
                    const statusColors = {
                    ...
                    <div class="flex justify-between items-center p-2 bg-background-dark rounded text-sm">
                        ...
                        <div class="flex gap-2 items-center">
                            ...
                            <span class="font-semibold py-1 px-2 rounded ${statusColors[status]}">${indicador.atual}</span>
                        </div>
                    </div>
                `}).join('')}
            </div>
...
function displayPriorityActions(alertasData) {
    const container = document.getElementById('priorityActions');
    if (!container) return;

    const statusColors = {
        'critico': { border: 'border-l-red-500', badge: 'bg-red-500/20 text-red-400', icon: '🔴' },
        'alerta': { border: 'border-l-yellow-500', badge: 'bg-yellow-500/20 text-yellow-400', icon: '⚠️' },
        'pendente': { border: 'border-l-gray-500', badge: 'bg-gray-500/20 text-gray-400', icon: '⏳' }
    };

    container.innerHTML = alertasData.map(alerta => {
        const style = statusColors['alerta'];
        
        return `
        <div class="bg-white/5 border border-white/10 ${style.border} border-l-4 rounded-lg p-4">
            <div class="flex justify-between items-start gap-4 mb-3">
                <h3 class="text-white font-semibold text-base">${alerta.alerta}</h3>
                <span class="font-semibold py-1 px-3 rounded text-sm ${style.badge}">
                    ${style.icon} Alerta
                </span>
            </div>
            <div class="bg-background-dark p-3 rounded text-sm text-gray-300">
                <span class="text-primary font-semibold">Ação:</span> ${alerta.acao}
            </div>
        </div>
    `}).join('');
}
...
function displayTimeline(timelineData) {
    ...
    container.innerHTML = timelineData.map((periodo) => `
        <div class="timeline-card bg-gradient-to-br ${gradients[periodo.cor] || 'from-gray-500 to-gray-600'} text-white rounded-xl p-6 relative overflow-hidden">
            <h3 class="text-xl font-bold mb-2">${periodo.periodo}</h3>
            <p class="text-sm opacity-90 mb-4">🎯 ${periodo.titulo}</p>
            <ul class="flex flex-col gap-2">
