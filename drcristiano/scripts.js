...
async function loadChecklist() {
    try {
        ...
        const savedChecklist = localStorage.getItem('drcristiano-checklist');
...
function showCelebration() {
    ...
    if (progressText && !progressText.classList.contains('celebrated')) {
        ...
        setTimeout(() => {
            ...
            const items = JSON.parse(localStorage.getItem('drcristiano-checklist') || '[]');
            const completed = items.filter(item => item.completed).length;
            const total = items.length;
...
function updateChecklistItem(index, completed) {
    ...
    if (savedChecklist) {
        ...
        localStorage.setItem('drcristiano-checklist', JSON.stringify(data));
...
async function resetChecklist() {
    if (confirm('Tem certeza que deseja resetar todo o progresso do checklist?')) {
        try {
            // Limpar localStorage
            localStorage.removeItem('drcristiano-checklist');
        ...
        <div class="bg-white/5 border border-white/10 rounded-lg p-4 flex flex-col">
            ...
            <div class="flex flex-col gap-2 flex-1">
                ${categoria.indicadores.map(indicador => {
                    ...
                    return `
                    <div class="flex justify-between items-center gap-2 p-2 bg-background-dark rounded text-sm min-h-[40px]">
                        <span class="text-gray-300 font-medium">${indicador.nome}</span>
                        <div class="flex gap-2 items-center flex-shrink-0 ml-auto">
                            <span class="text-gray-400 text-xs whitespace-nowrap">Meta: ${indicador.meta}</span>
                            <span class="font-semibold py-1 px-2 rounded whitespace-nowrap ${statusColors[indicador.status]}">${indicador.atual}</span>
                        </div>
                    </div>
                `}).join('')}
            </div>
